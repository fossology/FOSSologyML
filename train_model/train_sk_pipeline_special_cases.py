...
"""
Script that creates a sklearn special cases pipeline in following way:
...
from rigel.pipeline.sk_pipeline import SKLearnProblem
...
def load_special_case_data(max_samples, basenames, exclude_tags):
    x_train = {}
    y_train = {}
    with ProcessPoolExecutor(max_workers=n_cores) as executor:
        for basename, result in zip(basenames,
                                    executor.map(partial(load_single_special_case_data, max_samples, exclude_tags),
                                                 basenames)):
            x_train[basename], y_train[basename] = result
    return x_train, y_train


def load_single_special_case_data(max_samples, exclude_tags, basename):
    logger.info(f"Loading {max_samples} samples of {basename} special cases")
    with db.connect():
        docs = Documents.Conclusion.objects(mapped_licenses=re.compile(f"^{basename}"),
                                            tags__nin=exclude_tags).only('license_related_words',
                                                                         'licenses',
    ...
    if training_tuples:
        x_train, y_train_unfiltered = zip(*training_tuples)
        # filtering: we have to make sure we only show relevant licenses to the model that are special cases for this
        # basename. It is irrelevant whatever other additional licenses the current file might have, so we remove them
        y_train = []
        for y in y_train_unfiltered:
            y_train += [list(filter(lambda l: l.startswith(basename), y))]
    else:
        x_train, y_train = [], []

    logger.info(f"Train documents: {docs._mongo_query}, count: {len(x_train)}")
    return x_train, y_train
...
def train_special_sk_pipeline(pipeline, max_samples, train_in_parallel=False, exclude_data_for_benchmark=True):
    ...
    y_single_label = list(map(lambda y: [generalize_special_cases(l) for l in y], y_single_label))

    x_multi_label, y_multi_label = load_data(max_samples=max_samples,
                                             tags__all=[Tag.MULTIPLE_MAPPED_LICENSE.value],
                                             tags__nin=excluded_tags)
    y_multi_label = list(map(lambda y: [generalize_special_cases(l) for l in y], y_multi_label))
    ...
    x_special_cases, y_special_cases = load_special_case_data(max_samples, SPECIAL_CASES,
                                                              excluded_tags)
...
if __name__ == '__main__':
    ...
    add_file_handler_to_logger(logger, scenario_dir, 'train_sk_pipeline_special_cases')
    ...
    try:
        ...
        pipeline = SpecialSKLearnPipeline(ConfigParser(), data_loader)

        train_special_sk_pipeline(pipeline,
                                  max_samples=max_samples,
                                  train_in_parallel=True,
                                  exclude_data_for_benchmark=False)
